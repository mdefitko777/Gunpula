...
搜索洞察需要额外执行 `docs/supabase-search-insights.sql`。它只记录零结果的搜索词、主题、界面语言、次数和时间，不记录用户 ID、邮箱、收藏或浏览记录。App 必须登录后才能上报，只有 CMS 管理员可以读取。
...
“搜索洞察”按频次显示 App 中没有结果的词。先选择正确的作品/系列分类，再点“加入别名”；修改会进入草稿，预览确认后随下一次 CMS 发布生效。商品总览使用与 App 相同的搜索引擎，可直接查四语名称、简称、作品、产品线、JAN/商品编号和近似拼写。
