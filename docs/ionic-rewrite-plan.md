...
- [x] 设置、主题（浅 / 深 / 跟随系统）
- [x] i18n 四语言
- [x] 目录：多维筛选（系列/产品线/年份/限定/价格区间）+ 搜索
- [x] 账号同步：邮箱 OTP 登录、共享空间/邀请码、好友主页看对方收藏
- [x] 世界配色贯彻（五世界主题色映射到 --ion-color-primary，全组件跟随）
- [x] PWA（service worker、离线、安装、manifest/图标）
- [x] 商品类型筛选（grades 分类表 → 拼装/成品/扭蛋/食玩…）
- [x] BBX 图鉴点亮：完成品陀螺按 set code 关联目录收藏（部品无目录条目，保持不亮）

## 上线步骤（待执行，需用户确认）

1. `cd ionic && npm run build` → 产物在 `ionic/dist/`
2. 用 `ionic/dist/` 的内容替换 `app/`（保留 `app/` 里数据无关的东西？——不，构建产物自包含）
3. 数据路径：生产构建 `../data` 相对 `/Gunpula/app/` → `/Gunpula/data/`，已存在
4. Capacitor `server.url` 不变（仍 `/Gunpula/app/`）
5. 合并 `rewrite-ionic` → `main`，GitHub Pages 自动发布
6. APK 下次打开自动加载新版
